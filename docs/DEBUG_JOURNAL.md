...
| **Hypothesis** |The TypeScript type definition for `Shipment` expects `cargo_details` to be an object with the shape:

{ item: string, weight_kg: number } | null

However, after inspecting the `shipments` table directly in Supabase, the `cargo_details` column is stored as a JSONB array containing an object, for example:

[{ "item": "Laptop Batch A", "weight_kg": 120 }]

This means the actual runtime data returned by Supabase is an array, while the frontend TypeScript type assumes it is a single object. Because the TanStack Table cell renderer attempts to access `cargo.item` and `cargo.weight_kg`, the values appear undefined when the value is actually an array, resulting in an empty Cargo column.

The repository already includes a utility function `normalizeCargoDetails` designed to convert the JSON array format into a single object, but this function is not currently used when rendering the TanStack Table column. Therefore, the issue is likely caused by a mismatch between the database JSON structure and the expected TypeScript type, combined with the missing normalization step before rendering.|
